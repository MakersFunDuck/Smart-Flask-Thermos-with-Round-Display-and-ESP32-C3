...
void CST816D::interrupt_prepare(void)  //not working
{
   for (uint8_t reg = 0x00; reg <= 0xFF; reg++) {
    i2c_write(reg,0x00);
    delay(2);
    // Stop if register 0xFF is reached
    if (reg == 0xFF) {
    ...
    // Initialize CST816D to enable touch detection
  //  i2c_write(0x03, 0x01); //
    //i2c_write(0xA0, 0x00); // Optional: Reset touch status (clear any previous touch info)
    //i2c_write(0xA3, 0x01); // Set interrupt pin polarity to active high (when touch is detected)
    //i2c_write(0xA6, 0x01); // Enable touch detection functionality
    //i2c_write(0xA0, 0x00); // Optional: Reset touch status (clear any previous touch info)
